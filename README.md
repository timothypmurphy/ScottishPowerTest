...
check the compulsory fields are supplied when POSTing new meter readings. Currently, they do not provide a meaningful error
...
this challenge, after I remembered about the existence of these data types while I was struggling to divide a long by an int. My thinking was that meter readings,
...
### Reading Repository
Firstly, something which bothered was the name of the methods in the ReadingRepository class.
...
current implementation pulls the readings of a particular type for all accounts in the DB and then calculates a global average.
...
thrown as well as testing different branches that the individual methods can take. I would like to have completed a full unit test suite, writing
tests for the controller class and well as the repository classes. I also think the tests I have written are not the cleanest, so given 
